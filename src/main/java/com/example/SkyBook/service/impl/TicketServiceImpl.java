...
public class TicketServiceImpl implements TicketService {
    ...
    @Override
    public TicketResponseDto getTicketById(Long id, String requesterEmail) {
        Ticket ticket = ticketRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Ticket not found"));
        assertOwnerOrAdmin(ticket, requesterEmail);
        return TicketMapper.toResponseDto(ticket);
    }

    @Override
    public List<TicketResponseDto> getAllTickets(String requesterEmail) {
        User requester = userRepository.findByEmail(requesterEmail)
                .orElseThrow(() -> new ResourceNotFoundException("User not found."));

        List<Ticket> tickets = requester.getRole() == Role.ADMIN
                ? ticketRepository.findAll()
                : ticketRepository.findByBooking_User_Id(requester.getId());

        return tickets.stream().map(TicketMapper::toResponseDto).toList();
    }

    @Override
    public TicketResponseDto getTicketByNumber(String ticketNumber, String requesterEmail) {
    ...
    private void assertOwnerOrAdmin(Ticket ticket, String requesterEmail) {
        User requester = userRepository.findByEmail(requesterEmail)
                .orElseThrow(() -> new ResourceNotFoundException("User not found."));
